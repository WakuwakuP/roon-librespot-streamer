...
1. **Dockerfile** (Default) - Uses pre-built librespot binaries for faster builds
   ...
   - Uses librespot latest from dev branch
...
**解決策 (Solution)**:
1. Update to the latest version of this project (uses librespot latest from dev branch)
...
**注意 (Note)**: This project now uses librespot latest from the dev branch, which includes the most recent fixes for track availability and playback issues.
